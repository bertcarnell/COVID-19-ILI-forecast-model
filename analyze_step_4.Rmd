...
today_date <- "2020-04-03" # before Sunday, best if Friday
...
Check that the bin probabilities sum to 1: `r assertthat::assert_that(all(abs(apply(arima2_fcst_result$p_bin, 2, sum) - 1) < 1E-6))`

Check that the expected value of the bins is close to the point estimates:

- Expected value: `r round(c(matrix(reporting_bin_centers, nrow = 1, byrow = TRUE) %*% arima2_fcst_result$p_bin), digits = 2)`
- Point Est:  `r round(arima2_fcst_result$point, digits = 2)`
...
# first week of actual COVID cases to 1 week past the current ILI week
...
assertthat::assert_that(max(covid_daily_state_post$mmwrweek) == current_ili_week + 1)

covid_daily_state_pred <- covid_daily_state_pred_data %>%
  group_by(mmwrweek) %>%
  summarise(pred = max(pred),
            pred_ili = max(pred) * mean_inflation) # percent scale
assertthat::assert_that(nrow(covid_daily_state_pred) == WEEKS_AHEAD - 1)
assertthat::assert_that(min(covid_daily_state_pred$mmwrweek) == current_ili_week + 2)
```
...
plot_data <- data.frame(
  ...
  week = rep(paste0("Week Ahead ", (6 - k_col + 1):6), each = nrow(arima2_fcst_result_plus$p_bin))
)
plot_data2 <- data.frame(
  ili = arima2_fcst_result_plus$point,
  week = paste0("Week Ahead ", (6 - k_col + 1):6)
)
...
ggplot(plot_data) +  
  ...
  geom_vline(aes(xintercept = ili), data = plot_data2, col = "red") +
  labs(x = paste("Unweighted ILI in", hhs_regions_map$state[1]), y = "Density")
